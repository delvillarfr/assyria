...
estimation_data = readtable('estimation_data_all_directional.csv','TreatAsEmpty',{'.','NA'});

coordinates = readtable('coordinates_all.csv','TreatAsEmpty',{'.','NA'});
...
%%%%% Calculate T^(1/vartheta)'s and its SE
vartheta = 4;

auxdata.vartheta = vartheta;

T_grad_matrix = zeros((K+L), dim_theta);
...
report_table = array2table(coord(:,1:1), 'VariableNames', ...
    {'id'});

name_table = coordinates(:,{'id', 'oldid' , 'name'});
...
writetable(report_table, 'report_table_d_whitese.csv');
...
writetable(report_table, 'report_table_d_homose.csv');


save('workspace_d_step10.mat');
